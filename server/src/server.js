...
app.patch('/admin/products/:id', authRequired, roleRequired('Manager', 'Admin'), handle(async (req, res) => {
  const { id } = req.params;
  const { sku, name, description, unit, min_qty, lead_time_days } = req.body;
...
app.post('/admin/bins', authRequired, roleRequired('Manager', 'Admin'), handle(async (req, res) => {
  ...
  if (existing.length > 0) {
    return res.status(409).json({ error: 'Bin code already exists in this location' });
  }
...
// RECEIVE stock
app.post('/transactions/receive', authRequired, roleRequired('Worker', 'Manager', 'Admin'), idempotencyMiddleware, handle(async (req, res) => {
...
// SHIP stock
app.post('/transactions/ship', authRequired, roleRequired('Worker', 'Manager', 'Admin'), idempotencyMiddleware, handle(async (req, res) => {
    const { productId, binId, qty, reference, user } = req.body;
    if (!productId || !binId || !qty || qty <= 0) {
      return res.status(400).json({ error: 'productId, binId, positive qty required' });
    }
  ...
  handle(async (req, res) => {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO activity_logs(action_type, user_name, user_role, details)
        VALUES ($1, $2, $3, $4)`,
        ['TRANSFER', user, req.user.role, JSON.stringify({ productId, fromBinId, toBinId, qty, reference })]
      );
...
// Low stock view
app.get('/admin/low-stock', authRequired, roleRequired('Manager', 'Admin'), handle(async (req, res) => {
